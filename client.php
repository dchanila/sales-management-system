...
$sundaynextweek=date("Y-m-d",strtotime('sunday next week'));
...
$sqllogs="UPDATE userz SET state ='Online' WHERE username = '$username_logs' ";
$db->insert($sqllogs);
header("Location: ../dashboard_user/dashboard_option");
exit();
}
...
//view member
$member_views = $db->fetch(" SELECT * FROM userz");
//view members logs
...
$id = $_GET['del_product'];
$sql="DELETE FROM production_cost WHERE id=$id";
$db->insert($sql);
header("Location: ../add_cost_of_production?product=$product");
exit();
}
...
$avlbles='';
$newavlbles='';
if(isset($_GET['delete_prodction'])){
$id = $_GET['delete_prodction'];
$product_produced_del = $db->fetch("SELECT * FROM production WHERE id=$id");
if($product_produced_del->num_rows > 0){
while($row = $product_produced_del->fetch_assoc()) {
$hold_actz=$row['actual_no_of_blocks_produced'];
$prodcts=$row['product'];  
}
}
$product_produced_delz = $db->fetch("SELECT * FROM product WHERE product_name='$prodcts'");
if($product_produced_delz->num_rows > 0){
while($row = $product_produced_delz->fetch_assoc()) {
$prdced=$row['produce'];
$avlbles=$row['total'];  
}
}
$newavlbles=$avlbles-$hold_actz;
$newprdced=$prdced-$hold_actz;
$updateproductz="UPDATE product SET produce='$newprdced',total ='$newavlbles' WHERE product_name ='$prodcts'";
$db->insert($updateproductz);
$sql="DELETE FROM production WHERE id=$id";
$db->insert($sql);
header("Location: ../blocks_production?product=$prodcts");
exit();
}
...
if ($totalexpblok==0 or $totalactblokz==0 ) {
$percentblock=0;
}else{
...
$totalexpblokweekly=0;
$totalactblokweekly=0;
...
if ($totalexpblokweekly==0 or $totalactblokweekly==0 ) {
$totalblockspercent=0;
}else{
...
if ($totalexpblokmonthly==0 or $totalactblokmonthly==0 ) {
$totalblockspercentmonthly=0;
$totalactblokmonthly=0;
}else{
...
if ($totalexppevnt==0 or $totalactpevnt==0 ) {
$percentpevnt=0;
}else{
...
$totalexppavementmonthly=0;
$totalactpavementmonthly=0;
...
if ($totalexppavementmonthly==0 or $totalactpavementmonthly==0 ) {
$totalpavementpercentmonthly=0;
}else{
...
$recordfullname= mysqli_fetch_array( $reco);
$fullnames=$recordfullname['first_name']." ".$recordfullname['last_name'];
$sql= "INSERT INTO comment(reasons_id,from_table,reasons,comment,username) VALUES('$reason_id','production','$reason','$comment','$fullnames');";
$fullnames= $username_logs;
...
$recordfullname= mysqli_fetch_array( $reco);
$fullnames=$recordfullname['first_name']." ".$recordfullname['last_name'];
$sql= "INSERT INTO comment(reasons_id,from_table,reasons,comment,username) VALUES('$reason_id','owed_customer','$reason','$comment','$fullnames');";
$fullnames= $username_logs;
$sqls= "INSERT INTO logs (username,activity) VALUES('$username_logs','comment on daily reason');";
$db->insert($sql);
$db->insert($sqls);
header("Location: ../comment?owed_comment=$reason_id");
exit(); 
}
...
$percentz=round(($amount_paid/$subtotal_amount)*100);
$remaining=$subtotal_amount-$amount_paid;
...
header("Location: ../sales?product=$product&&errorsales=$saleserror");
}else{
$sql= "INSERT INTO sales(dates,product,open_with,produce,sold,price_per_each,subtotal_amount,amount_paid,percentz,remaining,closed_with,order_id) VALUES('$dates','$product','$open_with','$produce','$sold','$price_per_each','$subtotal_amount','$amount_paid','$percentz','$remaining','$closed_with','$order_id');";
$db->insert($sql);
$updateproduct="UPDATE product SET   total ='$closed_with' WHERE product_name = '$product' ";
$db->insert($updateproduct);

$statusupdate="";
$updateorder="UPDATE orders SET statuz ='delivered' WHERE order_id ='$order_id'";
$db->insert($updateorder);

$fullnames= $username_logs;
$sqls= "INSERT INTO logs (username,activity) VALUES('$username_logs','add daily $product sales');";  
$db->insert($sqls);
if ($amount_paid <$subtotal_amount ) {
$product_producedzz = $db->fetch(" SELECT * FROM sales");
if($product_producedzz->num_rows > 0){
while($row = $product_producedzz->fetch_assoc()) {
$id_salz=$row['sales_id'];    
}
header("Location: ../add_owed_custome?idz=$id_salz&&username=$username_logs");
exit();
}}else{ 
...
if (isset($_POST['owed_customer'])) {
$id_salxz=$_POST['id_salx'];
$datez=$_POST['datez'];
$fname=$_POST['fname'];
$productxz=$_POST['productx'];
$soldxz=$_POST['soldx'];
$price_per_eachxz=$_POST['price_per_eachx'];
$subtotal_amountxz=$_POST['subtotal_amountx'];
$amount_paidxz=$_POST['amount_paidx'];
$percentzxz=$_POST['percentzx'];
$remainingxz=$_POST['remainingx'];
$usernamez=$_POST['username'];
$commentz=$_POST['comment'];
$sqlz= "INSERT INTO owed_customer(dates,customer_name,product,sales_id,quantity_taken,price_per_each,total,paid,percent,remain,commentz,username) VALUES('$datez','$fname','$productxz','$id_salxz','$soldxz','$price_per_eachxz','$subtotal_amountxz','$amount_paidxz','$percentzxz','$remainingxz','$commentz','$usernamez');";
$db->insert($sqlz);
header("Location: ../owed_customer");
exit();
}

if (isset($_POST['pay_owed'])) {
$pay_id=$_POST['pay_id'];
$total=$_POST['total'];
$paid=$_POST['paid'];
$addPayz=$_POST['addPayz'];
$salz_id=$_POST['salz_id'];
$sumz=$paid+$addPayz;
$percee=round(($sumz/$total)*100);
$remainz=$total-$sumz;
$updatesalz="UPDATE sales SET   amount_paid='$sumz',percentz='$percee',remaining='$remainz' WHERE sales_id=$salz_id";
$db->insert($updatesalz);
$updateowed="UPDATE owed_customer SET paid='$sumz',percent='$percee',remain='$remainz' WHERE sales_id=$salz_id";
$db->insert($updateowed);
header("Location: ../owed_customer");
exit();
}
...
///////////////////
$saleserr='';
if(isset($_GET['errorsales'])){
$saleserr = $_GET['errorsales'];
...
$id = $_GET['delete_sales'];
$product_producedz= $db->fetch("SELECT * FROM sales WHERE sales_id=$id");
if($product_producedz->num_rows > 0){
while($row = $product_producedz->fetch_assoc()) {
$productz=$row['product'];
$soldz=$row['sold']; 
$order_idx=$row['order_id'];    
}
}
$product_producedz= $db->fetch("SELECT * FROM sales WHERE sales_id > $id and product='$productz'");
if($product_producedz->num_rows > 0){
while($row = $product_producedz->fetch_assoc()) {
...
for ($i=$id; $i <=$salesId ; $i++) { 
$product_producedzz= $db->fetch("SELECT * FROM sales WHERE sales_id = $i and product='$productz'");
if($product_producedzz->num_rows > 0){
while($row = $product_producedzz->fetch_assoc()) {
$salesIdz=$row['sales_id']; 
$clozdwith=$row['closed_with'];  
}
}
$slz=$soldz+$clozdwith;
$updatesales="UPDATE sales SET closed_with ='$slz' WHERE sales_id=$i";
$db->insert($updatesales);
}
$product_produced_delz = $db->fetch("SELECT * FROM product WHERE product_name='$productz'");
if($product_produced_delz->num_rows > 0){
while($row = $product_produced_delz->fetch_assoc()) {
$avlblez=$row['total'];  
}
}
$updateopwithz=$avlblez+$soldz;
$updateproduct="UPDATE product SET total ='$updateopwithz' WHERE product_name ='$productz'";
$db->insert($updateproduct);
$updateorders="UPDATE orders SET statuz ='confirmed' WHERE order_id=$order_idx";
$db->insert($updateorders);
$sql="DELETE FROM sales WHERE sales_id=$id";
$db->insert($sql);
$sqlz="DELETE FROM owed_customer WHERE sales_id=$id";
$db->insert($sqlz);
header("Location: ../sales?product=$productz");
exit();
}
...
$owed_customerz = $db->fetch(" SELECT * FROM  owed_customer WHERE remain > 0 or remain < 0");
...
if ($open_withblocks==0 or $productblocks==0) {
$open_withblocks=0;
$productblocks=0;
...
//delete material procure
$materialtodel=0;
$material_typez=0;
$material_availablz=0;
if(isset($_GET['delete_material_procure'])){
$id = $_GET['delete_material_procure'];
$materialdel = $db->fetch("SELECT * FROM material_procure WHERE id=$id");
if($materialdel->num_rows > 0){
while($row = $materialdel->fetch_assoc()) {       
$materialtodel=$row['quantity'];
$material_typez=$row['material_type'];   
}}
$materialsprocurez = $db->fetch("SELECT * FROM material WHERE matarial_name='$material_typez'");
if($materialsprocurez->num_rows > 0){
while($row = $materialsprocurez->fetch_assoc()) {       
$material_availablz=$row['available'];   
}}
$newclozdprocure=$material_availablz-$materialtodel;
$newmatrialavlbzz="UPDATE material SET   available ='$newclozdprocure' WHERE matarial_name = '$material_typez'";
$db->insert($newmatrialavlbzz);
$sql="DELETE FROM material_procure WHERE id=$id";
$db->insert($sql);
header("Location: ../material_procure_add?mname=$material_typez");
exit();
}
...
if ($usedmaterial < 0) {
$materialerror="Not enough ".$materialname." available is ".$material_availables." only , so go to material procurement and add ".$materialname." quantity";
header("Location: ../material_use_add?mname=$materialname&&matarialerror=$materialerror");
exit();
}else{
...
$matarialserr='';
if(isset($_GET['matarialerror'])){
$matarialserr = $_GET['matarialerror'];
...
$materialdel = $db->fetch(" SELECT * FROM materials WHERE id=$id");
if($materialdel->num_rows > 0){
while($row = $materialdel->fetch_assoc()) {       
$material_avlble=$row['used'];
$material_nm=$row['name'];   
}}
$materialdelcheck = $db->fetch(" SELECT * FROM materials WHERE id > $id");
if($materialdelcheck->num_rows > 0){
while($row = $materialdelcheck->fetch_assoc()) {       
$material_id=$row['id'];   
}}
for ($i=$id; $i <=$material_id ; $i++) { 
$materialnewadd = $db->fetch(" SELECT * FROM materials WHERE id = $i");
if($materialnewadd ->num_rows > 0){
while($row = $materialnewadd ->fetch_assoc()) {       
$material_clozd=$row['total'];   
}}
$newavlbz=$material_avlble+$material_clozd;
$newmatrialzavlb="UPDATE materials SET   total ='$newavlbz' WHERE id=$i";
$db->insert($newmatrialzavlb);
}
$materialsusedz = $db->fetch("SELECT * FROM material WHERE matarial_name='$material_nm'");
if($materialsusedz->num_rows > 0){
while($row = $materialsusedz->fetch_assoc()) {       
$material_availablz=$row['available'];   
}}
$newclozd=$material_availablz+$material_avlble;
$newmatrialavlb="UPDATE material SET   available ='$newclozd' WHERE matarial_name = '$material_nm' ";
$db->insert($newmatrialavlb);
$sql="DELETE FROM materials WHERE id=$id";
$db->insert($sql);
header("Location: ../material_use_add?mname=$material_nm");
exit();
}
...
//delete money received
if(isset($_GET['delete_root'])){
$id = $_GET['delete_root'];
...
$id = $_GET['delete_fuel'];
$sql="DELETE FROM truck_fuel WHERE id=$id";
$db->insert($sql);
header("Location: ../truck_add_fuel?tname=$truckname");
exit();
}
...
$id = $_GET['delete_pay'];
$sql="DELETE FROM truck_payment WHERE id=$id";
$db->insert($sql);
header("Location: ../truck_add_pay?tname=$truckname");
exit();
}
...
$sql="DELETE FROM truck_service WHERE id=$id";
$db->insert($sql);
header("Location: ../truck_add_service?tname=$truckname");
exit();
}
...
//delete fuel site
if(isset($_GET['delete_fuel_site'])){
$id = $_GET['delete_fuel_site'];
$sql="DELETE FROM fuel_site WHERE id=$id";
$db->insert($sql);
header("Location: ../site_fuel_add");
exit();
}
...
//delete service site
if(isset($_GET['delete_service_site'])){
$id = $_GET['delete_service_site'];
$sql="DELETE FROM service_site WHERE id=$id";
$db->insert($sql);
header("Location: ../site_service_add");
exit();
}
...
//delete pay site
if(isset($_GET['delete_pay_site'])){
$id = $_GET['delete_pay_site'];
$sql="DELETE FROM site_payment WHERE id=$id";
$db->insert($sql);
header("Location: ../site_pay_add");
exit();
}
...
//delete estimation daily
if(isset($_GET['delete_estimation_daily'])){
$id = $_GET['delete_estimation_daily'];
...
$sundayz=date("Y-m-d",strtotime("sunday this week"));
if ($datex=='$saturday' or $datex=='$sundayz') {
$sqlz="INSERT INTO estimate (dates,to_date,detail,quantity,price_per_each,total_amount) VALUES('$mondaynextweek','$sundaynextweek','$detail','$quantity','$price_per_each','$total_amount');";
$db->insert($sqlz);
$fullnames= $username_logs;
$sqlsz= "INSERT INTO logs (username,activity) VALUES('$username_logs','add  estimetion cost detail');";  
$db->insert($sqlsz);
...
//delete estimation weekly
if(isset($_GET['delete_estimation_weekly'])){
$id = $_GET['delete_estimation_weekly'];
$sql="DELETE FROM estimate WHERE id=$id";
$db->insert($sql);
header("Location: ../site_estimation_weekly");
exit();
...
//view order
$view_order = $db->fetch(" SELECT * FROM orders WHERE order_type='$product' and statuz='not confirmed'");
...
$paid=round($paid, 1); 
if ($total_amounts > $realamountpaid) {
$status = "not confirmed";
}else{
$status = "confirmed";
}
$updateorder="UPDATE orders SET amount_paid ='$realamountpaid',statuz='$status',remaining ='$remaining',paid='$paid' WHERE order_id ='$order_id'";
$db->insert($updateorder);
$fullnames= $username_logs;
...
//report sales daily
$view_dail_reportsales = $db->fetch("SELECT * FROM `sales` WHERE dates='$dates1' and product='$product'");
$view_dail_reportsalesz = $db->fetch("SELECT dates,SUM(sold)as sold,sum(subtotal_amount)as subtotal_amount,sum(amount_paid) as amount_paid,sum(remaining)as remaining FROM `sales` WHERE dates='$dates1' and product='$product'");
//weekly sales report
$view_weekly_reportsales = $db->fetch("SELECT * FROM `sales` WHERE dates BETWEEN '$monday' and '$sunday' and product='$product'");
$view_weekly_reportsalesz = $db->fetch("SELECT dates,SUM(sold)as sold,sum(subtotal_amount)as subtotal_amount,sum(amount_paid) as amount_paid,sum(remaining)as remaining FROM `sales` WHERE dates BETWEEN '$monday' and '$sunday' and product='$product' GROUP by dates");
///production
...
$view_monthly_reportsalesdashboard = $db->fetch("SELECT * FROM `sales` WHERE dates BETWEEN '$firstday' and '$lastday' and product='$product'");
$view_monthly_reportsales = $db->fetch("SELECT t.dates,sum(sold) as sold,sum(subtotal_amount) as subtotal_amount,sum(amount_paid) as amount_paid,sum(remaining)as remaining, CASE WHEN dates =1 then 'week1' WHEN dates = 2 then 'week2' WHEN dates = 3 THEN 'week3' WHEN dates = 4 THEN 'week4'  WHEN dates = 5 then 'week5' END as weeks from (SELECT (WEEK(dates) - WEEK(DATE_FORMAT(dates,'%Y-%m-01')))+1 as dates,sum(sold) as sold,sum(subtotal_amount) as subtotal_amount,sum(amount_paid) as amount_paid,sum(remaining)as remaining FROM `sales` WHERE dates BETWEEN '$firstday' and '$lastday' and product='$product' group by dates)t GROUP by t.dates");
...
$look=$_POST['look'];
if($look=='production'){ 
header("Location: ../search_option?searchFor=$look");
exit(); 
}
if ($look=='sales') {
header("Location: ../search_sales?searchFor=$look");
exit(); 
}
if ($look=='vehicle') {
header("Location: ../search_root?searchFor=$look");
exit(); 
}
if ($look=='orders') {
header("Location: ../search_order?searchFor=$look");
exit(); 
}
if ($look=='budget') {
header("Location: ../search_budget?searchFor=$look");
exit(); 
}
if ($look=='estimate') {
header("Location: ../search_estimate?searchFor=$look");
exit(); 
}
if ($look=='consumption') {
header("Location: ../search_consumption?searchFor=$look");
exit(); 
}
}
$prodct='';
$datez='';
$datec='';
if (isset($_POST['checkz'])) {
$prodctz=$_POST['prodctz'];
$datez=$_POST['datez'];
$datec=$_POST['datec'];
if ($datec=='') {
$datec=$datez;
...
$looking=$_POST['searc'];
if ($looking=='production') {
header("Location: ../search?searchFor=$looking&&product=$prodctz&&dateFrom=$datez&&dateTo=$datec");
exit(); 
}if ($looking=='sales') {
header("Location: ../search_sales_opt?searchFor=$looking&&product=$prodctz&&dateFrom=$datez&&dateTo=$datec");
exit(); 
} 
if ($looking=='vehicle') {
header("Location: ../search_root_option?searchFor=$looking&&product=$prodctz&&dateFrom=$datez&&dateTo=$datec");
exit(); 
}
if ($looking=='orders') {
header("Location: ../search_order_option?searchFor=$looking&&product=$prodctz&&dateFrom=$datez&&dateTo=$datec");
exit(); 
}
if ($looking=='budget') {
header("Location: ../search_budget_option?searchFor=$looking&&product=$prodctz&&dateFrom=$datez&&dateTo=$datec");
exit(); 
}
if ($looking=='estimate') {
header("Location: ../search_estimate_option?searchFor=$looking&&product=$prodctz&&dateFrom=$datez&&dateTo=$datec");
exit(); 
}
if ($looking=='consumption') {
header("Location: ../search_consumption_option?searchFor=$looking&&product=$prodctz&&dateFrom=$datez&&dateTo=$datec");
exit(); 
}
}
?>