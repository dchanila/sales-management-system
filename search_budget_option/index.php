...
if ($dateFrom==''){
$view_money_receivedz= $db->fetch("SELECT * FROM $looking WHERE receive_from LIKE '%$product%' or  receive_from LIKE '%$product' or receive_from LIKE '$product%' ORDER by dates DESC");
}elseif ($product=='') {
	$view_money_receivedz= $db->fetch("SELECT * FROM $looking WHERE dates BETWEEN '$dateFrom' AND '$dateTo' ORDER by dates DESC");
}else{
	$view_money_receivedz= $db->fetch("SELECT * FROM $looking WHERE dates BETWEEN '$dateFrom' AND '$dateTo' AND receive_from LIKE '%$product%' ORDER by dates DESC");
}
